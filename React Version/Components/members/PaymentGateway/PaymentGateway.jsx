...
import "../../../static/css/members/paymentGateway/paymentGateway.css";
import {jobDetails, makePayment}  from '../../utils/APIRoutes'
import { useNavigate, useParams } from 'react-router-dom';

import GlobalNavBar from '../../GlobalNavbar'
import $ from 'jquery'
export default function PaymentGateway({currentUser, setCurrentUser}){
    ...
    const [autoCardNumber, setAutoCardNumber] = useState()
  ...
  $(document).ready(function() {
    $(".question-mark").hover(function() {
      $('.msg').fadeIn('fast').addClass('show');
    }, function() {
      $('.msg').fadeOut('fast').removeClass('show');
    });
  });
      ...
      .catch((error) => {
        ...
        const fetchInterval = setInterval(() => {
          ...
          if (fetchCount <= 0) {
            ...
            setLoadingMessage("Internal Server Error, try again later");
            return;
          } else {
            axios
              .get(`${jobDetails}/${jobID}`)
              .then((response) => {
                ...
                // Stop the interval once data is successfully fetched
                clearInterval(fetchInterval);
              })
              .catch((error) => {
                console.log('Error Fetching job: ' + error);
  ...
  function handleCardNumberChange(event) {
    ...
    // Limit the card number to a maximum of 16 digits
    const limitedNumber = digitsOnly.slice(0, 16);

    // Split the string into groups of 4 characters
    const formattedNumber = limitedNumber.replace(/(\d{4})(?=\d)/g, "$1 ");

    setAutoCardNumber(formattedNumber);
  }

  async function validation(cardNumber, expiryDate, securityCode, firstName, lastName, errors){
    if (!cardNumber || !expiryDate || !securityCode || !firstName || !lastName) {
      setError("Do not leave any fields blank.");
      return
    } 
      const formattedCardNumber = cardNumber.replace(/\s/g, "");
      if (isNaN(formattedCardNumber) || formattedCardNumber.length !== 16) {
        errors++
        setCardMessage("Enter a valid card number!");
      }

      if (isNaN(securityCode)) {
        errors++
        setSecurityMessage("Invalid Format!");
      }
      const dateFormatRegex = /^\d{2}\/\d{2}\/\d{2}$/;
      const nameRegex = /^[A-Za-z]+$/;

      if (!dateFormatRegex.test(expiryDate)) {
        setDateMessage("Invalid Date Format!");
        errors++
      }
  ...
  async function dataErrors(data){
    if(data.error){
      setError(response.data.error)
      return
    }
    if(data.cardNumberError){
      setCardMessage(data.cardNumberError)
    }
    if(data.expiryDateError){
      setDateMessage(data.expiryDateError)
    }
    if(data.securityCodeError){
      setSecurityMessage(data.securityCodeError)
    }
  ...
  async function handlePayment(event){
    ...
    await validation(cardNumber, expiryDate, securityCode, firstName, lastName, errors)
      if(errors === 0){
        const response = await axios.post(makePayment, 
          {
            firstName, lastName, securityCode, expiryDate, cardNumber,
            clientEmail: currentUser.email,
            client_id: currentUser._id,
            jobFee, totalFee, serviceFee, 
            jobThumbnail: job[0].thumbnail,
            freelancer_id: job[0].seller_id
          })
          const data =response.data
          await dataErrors(data)
            setIsLoading(false)
            if(data.message) {
  ...
  return (
    <>
      ...
      {job && job.length > 0 && currentUser ? (
        <form className="payment-gateway-main-container" onSubmit={handlePayment}>
          ...
          <section className="payment-gateway">
            ...
            <div className="cards">
                <input type="radio" name="cardOption" id="cardOption" checked/>
          ...
          <section className="payment-details">
            ...
            <div className="job-title">
              ...
              <div className="job-thumbnail">
                <img src={`http://localhost:3000/images/${job[0].thumbnail}`} alt="" />
              </div>
            ...
            <div className="job-fee">
                <p className="cost-title">Job fee</p>
                <p className="cost">${jobFee.toFixed(2)}</p>
              </div>

              <div className="service-fee">
                <p className="cost-title">Service fee</p>
                <p className="cost">${serviceFee.toFixed(2)}</p>
              </div>

              <div className="total-fee">
                <p  className="cost-title">Total</p>
                <p className="cost">${totalFee.toFixed(2)}</p>
              </div>
            </div>

            <button type="submit" className="form-control confirm-payment-btn">{isLoading ? <div className="spinner" id="spinner"></div> : "Confirm & Pay"}

            </button>
            <p className="ssl"><i className="fa fa-lock"></i>SSL Secure Payment</p>
