...
export default function JobDetailsComponent(){
  ...
  for (const [star, count] of Object.entries(ratingCounts)) {
    ...
    useEffect(()=> {
      if ((jobs?.length ?? 0) > 0 && reportForm) {
        jobInfo.push({
          _id: jobs && jobs.length > 0 ? jobs[0]._id : null,
          job_title: jobs && jobs.length > 0 ? jobs[0].title : null,
          user_id: session?._id,
          freeLancerID: jobs && jobs.length > 0 ? jobs[0].freeLancerID : null,
        })
      }
          ...
          {jobs?.map((job, index) => (
            ...
            <div className="card" key={index++}>
              ...
              {job.freeLancerID !== session?._id && (
                ...
                <button 
                  className='form-control job-details-btn' 
                  style={{backgroundColor: "grey", color: "white"}}
                  onClick={()=> router.push('/auth/login')}
                  >
                  Log in to contact {job.username}
                </button>
              )}
                </>
              )}
